...
### 문제 풀이
입력 데이터 갯수의 크기만 주어져있는데 해당 값이 100이라서 O(n)이여도 상관없다고 판단하였다.<br>
해당 문제를 읽었을때 연결과 전파라는 키워드를 통해 해당 문제가 그래프 탐색 문제라고 판단하였고, 탐색 경로는 상관없으니 무한루프에 빠질수 있는 DFS보다 DFS를 사용하 문제를 풀이하였다.<br>
방향이 존재하지 않는 그래프이며 set 자료구조를 이용하여 그래프를 구현하였다.